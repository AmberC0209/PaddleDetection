...
### 4.1 模型微调
由于通用目标检测产线包含目标检测模块，如果模型产线的效果不及预期，那么您需要参考[目标检测模块开发教程](../module_usage/object_detection.md)中的[二次开发](../module_usage/object_detection.md#四二次开发)章节，使用您的私有数据集对目标检测模型进行微调。
